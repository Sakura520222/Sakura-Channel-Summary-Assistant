# 🌸 Sakura-Bot Wiki

欢迎来到 Sakura-Bot 项目文档中心！这里汇集了所有面向用户和开发者的详细文档。

---

## 📚 文档导航

### 👥 用户文档

面向使用 Sakura-Bot 的用户，包含安装、配置和使用指南。

- **[用户文档首页](User-Guide.md)** - 查看所有用户文档
- **[快速开始](User-Getting-Started.md)** - 快速部署和开始使用
- **[问答 Bot 指南](User-QA-Bot-Guide.md)** - 智能问答功能使用指南
- **[RAG 快速启动](User-RAG-Quickstart.md)** - 语义检索功能快速配置
- **[多轮对话功能](User-Multi-Turn-Conversation.md)** - 上下文感知对话说明
- **[投票功能](User-Poll-Feature.md)** - 频道投票配置指南
- **[行为准则](Code-of-Conduct.md)** - 社区贡献行为准则

### 👨‍💻 开发者文档

面向参与 Sakura-Bot 开发的贡献者，包含架构、实现和开发指南。

- **[开发者文档首页](Developer-Guide.md)** - 查看所有开发者文档
- **[开发快速开始](Developer-Getting-Started.md)** - 开发环境配置
- **[架构设计](Developer-Architecture.md)** - 系统架构和模块设计
- **[代码重构总结](Developer-Refactoring-Summary.md)** - 重构改进说明
- **[错误处理增强](Developer-Error-Handling-Improvements.md)** - 错误处理机制
- **[问答 Bot 实施](Developer-QA-Bot-Implementation.md)** - AI 功能实施文档
- **[RAG 技术文档](Developer-RAG-Feature-V3.md)** - RAG 技术实现
- **[Ruff 脚本指南](Developer-Ruff-Script-Guide.md)** - 代码质量工具
- **[CI/CD 工作流](Developer-Workflow-Improvements.md)** - 持续集成配置

---

## 🎯 快速查找

### 按主题查找

| 主题 | 用户文档 | 开发者文档 |
|------|---------|-----------|
| **安装配置** | [快速开始](User-Getting-Started.md) | [开发环境](Developer-Getting-Started.md) |
| **AI 功能** | [问答 Bot](User-QA-Bot-Guide.md) | [实施文档](Developer-QA-Bot-Implementation.md) |
| **RAG 功能** | [快速启动](User-RAG-Quickstart.md) | [技术文档](Developer-RAG-Feature-V3.md) |
| **投票功能** | [配置指南](User-Poll-Feature.md) | - |
| **代码质量** | - | [Ruff 指南](Developer-Ruff-Script-Guide.md) |
| **系统架构** | - | [架构设计](Developer-Architecture.md) |

### 按难度查找

| 难度 | 文档 |
|------|------|
| ⭐ **入门** | [快速开始](User-Getting-Started.md) · [RAG 快速启动](User-RAG-Quickstart.md) |
| ⭐⭐ **进阶** | [问答 Bot 指南](User-QA-Bot-Guide.md) · [投票功能](User-Poll-Feature.md) |
| ⭐⭐⭐ **高级** | [RAG 技术文档](Developer-RAG-Feature-V3.md) · [架构设计](Developer-Architecture.md) |

---

## 📖 文档结构说明

本文档采用 **Diátaxis 文档框架** 进行组织：

- **📘 教程 (Tutorials)** - 手把手的学习型指南
- **📗 操作指南 (How-to Guides)** - 解决具体问题的步骤说明
- **📙 参考资料 (Reference)** - 技术细节和 API 文档
- **📕 解释说明 (Explanation)** - 背景知识和概念讨论

### 文档分类标识

每个文档顶部都有分类标识：

> **分类**: 用户文档 / 开发者文档  
> **类型**: 教程 / 操作指南 / 参考资料 / 解释说明  
> **难度**: ⭐ / ⭐⭐ / ⭐⭐⭐  
> **更新时间**: YYYY-MM-DD

---

## 🔗 相关资源

- **[项目主页](https://github.com/Sakura520222/Sakura-Bot)** - GitHub 仓库
- **[README](../README.md)** - 项目说明文档
- **[变更日志](../CHANGELOG.md)** - 版本更新记录
- **[贡献指南](../CONTRIBUTING.md)** - 如何贡献代码

---

## 💡 获取帮助

如果您在使用过程中遇到问题：

1. 📖 查阅相关文档
2. 🔍 搜索 [GitHub Issues](https://github.com/Sakura520222/Sakura-Bot/issues)
3. 💬 参与 [GitHub Discussions](https://github.com/Sakura520222/Sakura-Bot/discussions)
4. 📧 发送邮件至 [sakura520222@outlook.com](mailto:sakura520222@outlook.com)

---

<div align="center">

**🌸 Sakura-Bot**

智能 Telegram 频道管理与问答系统

[⭐ Star](https://github.com/Sakura520222/Sakura-Bot) · [🍴 Fork](https://github.com/Sakura520222/Sakura-Bot/fork) · [📖 文档](Home.md)

Made with ❤️ by [Sakura520222](https://github.com/Sakura520222)

</div>