...
### 新增
- **频道ID标准化功能**：添加频道ID标准化机制，确保频道ID的一致性和可靠性
  - 在 `core/config.py` 中新增 `normalize_channel_id()` 函数，支持多种频道ID格式的自动转换（@username、完整URL、纯数字ID）
  - 自动移除末尾斜杠，统一使用标准化格式
  - 支持的格式转换：`@username` → `https://t.me/username`、`https://t.me/username/` → `https://t.me/username`、数字ID → 添加 `-100` 前缀
...
### 改进
- **频道ID处理一致性**：通过标准化函数，确保整个系统中频道ID格式的统一
- **数据完整性**：标准化后的频道ID避免了因格式不同导致的数据重复或丢失
- **MySQL 连接错误处理**：改进 MySQL 数据库管理器的错误处理逻辑，区分连接失败和其他数据库错误

### 技术实现
- **core/config.py**：新增 `normalize_channel_id()` 函数，更新相关函数以使用标准化频道ID
...
### 修复
- 修复因频道ID格式不一致导致的总结时间记录和查询问题
- 修复数据库中因频道ID格式不同导致的记录重复问题
