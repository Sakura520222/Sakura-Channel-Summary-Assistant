...
### 新增
- **转发配置热重载去重**：转发模块订阅配置变更事件时新增指纹去重，避免 WebUI 显式重载与文件监听重复触发造成重复刷新
- **转发源频道缓存优化**：重构转发源频道 ID 缓存刷新逻辑，配置变更后可更稳定地识别新增、删除或修改的源频道

### 改进
- **配置对象防御性拷贝**：转发处理器接收新配置时使用独立副本，降低外部引用修改导致状态污染的风险
- **异常处理增强**：优化转发配置复制和热重载过程中的异常日志，便于排查配置更新失败原因
...
### 新增
- **WebUI 配置保存即时生效**：WebUI 保存配置后显式触发配置热重载，使 AI 客户端、调度任务和转发规则更快应用新配置
- **转发回退机制**：频道消息转发新增 Bot 客户端到 UserBot/监听客户端的回退路径，提升 Bot 无法解析实体时的转发成功率
- **转发底栏占位符增强**：自定义底栏新增 `{source_link}`、`{assistant_bot}`、`{submission}` 等占位符，便于生成原文链接、助手 Bot 和投稿入口

### 改进
- **WebUI 转发表单优化**：转发规则编辑支持动态插入底栏占位符和模板，降低手动拼写错误概率
- **转发规则删除增强**：重构转发规则删除校验逻辑，增强定位说明和错误提示
...
### 新增
- **Main Bot 内联菜单**：欢迎消息和帮助消息新增内联键盘入口，管理员可通过按钮快速访问常用管理功能
- **转发命令权限检查**：转发相关管理命令统一增加管理员权限校验，防止非管理员修改转发规则或全局开关
- **WebUI 数据库管理**：新增 WebUI 数据库管理 API 与页面，支持表列表、分页查询、行增删改、清空数据并重启等管理操作
- **前端构建集成更新流程**：`/update`、`start.sh` 和 `start.bat` 相关更新流程纳入 Web 前端构建步骤，确保 WebUI 资源与后端版本一致

### 改进
- **数据库查询性能优化**：优化表行数统计查询，减少 WebUI 数据库页面加载时的重复查询开销
- **数据库安全校验增强**：数据库管理写操作增加表名和列名校验，降低动态 SQL 误用风险
- **版本号统一**：版本号来源迁移到 `core.__version__`，减少多处硬编码不一致问题
- **时间字段修复**：修复投稿审核时间字段的时区格式问题，并新增修复脚本以处理历史数据

### 修复
- **数据库插入结果处理**：处理 `lastrowid` 为空的情况，提升不同 MySQL 驱动/执行路径下的兼容性
- **错误传播一致性**：数据库管理接口补充 `ValueError` 传播，避免前端收到不明确的通用错误
...
### 新增
- **匿名投稿功能**：投稿审核系统新增匿名投稿模式，保护投稿者隐私
- **统计 API 接口**：新增总结统计 API 路由，支持获取总结数量、频道统计等数据
- **分页偏移量支持**：为总结查询添加分页偏移量参数，支持大数据量下的高效分页
- **向量存储管理 API**：新增向量存储管理路由模块，支持通过 WebUI 管理向量集合
  - 支持清空集合操作
  - 支持按条件删除文档
  - 支持分批删除，避免内存溢出

### 改进
- **投稿交互鲁棒性增强**：增强投稿处理器的异常处理和状态管理，提升交互稳定性
- **向量集合管理优化**：重构清空集合与删除文档逻辑，支持分批删除避免内存问题
- **结果去重优化**：优化 Agent 工具结果去重逻辑，避免依赖对象 id
...
### 新增
- **WebUI 命令中心模块**：新增命令中心 API 路由，支持通过 WebUI 执行 Bot 命令
- **RAG 工具增强**：
  - 新增频道列表、解析、最近总结等工具函数
  - 为工具和问答引擎添加帖子链接支持，答案可追溯至原始消息
  - 优化频道搜索与结果处理逻辑

### 改进
- **版本号统一**：统一版本号引用为 `core.__version__`，消除硬编码版本号
- **转发规则删除增强**：重构频道转发规则删除的验证逻辑，添加定位说明
- **结果去重优化**：优化 Agent 工具结果去重逻辑，避免依赖对象 id
...
### 新增
- **WebUI 管理面板**：集成 Vue.js 前端与 FastAPI API 服务器
  - JWT 认证和 SPA 路由支持
  - 仪表盘统计（总结数、频道数、转发数等）
  - 频道管理、调度配置、转发规则管理
  - 频道总结生成（可从 WebUI 直接触发）
  - 深色/浅色主题持久化切换
  - WebUI 触发 Bot 优雅重启（`os.execv` 替换进程）
  - 开发模式配置（可控的 dev 免认证登录）
- **配置热重载增强**：
  - 支持 AI 客户端动态更新
  - 支持调度任务动态更新
  - 重构日志级别热重载与非阻塞 Telegram 通知
- **审计日志功能**：新增审计日志和工具模块兼容层

### 改进
- **品牌标识更新**：更新品牌标识为樱花 Emoji 并调整样式
- **全局配置导入优化**：将全局配置导入方式改为模块导入，提升代码一致性
- **链接解析改进**：改进链接解析优先级与清理接口异常处理
- **前端优化**：移除 pinia 依赖，新增 CSS 工具类，优化分页加载
- **模块导入重构**：重构模块导入为惰性加载，提升启动性能

### 修复
- **仪表盘统计修复**：修正统计字段名从 `total_summaries` 改为 `total_count`
- **统计页面修复**：修复统计页面字段映射与空值处理问题
...
### 修复
- **消息解析模式规范化**：防止动态内容中特殊字符被误解析为Markdown/HTML
- **通知类型判定优化**：将 submission 通知的纯文本判断改为集合匹配，提升准确性
- **QA引擎日期格式修复**：修复摘要创建日期字段格式化时的类型错误
...
### 新增
- **投稿审核系统**：新增投稿功能，用户通过QA Bot深链接入口投稿，管理员审核后发布
  - 支持文本、图片、文件等多种内容类型
  - 投稿底栏支持助推和助手BOT链接
  - 新增 `/submit` 和 `/cancel_submit` 命令
- **Agentic RAG 循环模块**：新增智能体RAG循环，支持工具调用与固定管线双模式
- **双数据源语义搜索**：语义搜索同时检索总结和原始消息，显著提升搜索准确率
- **实时RAG处理器增强**：向量存储和实时RAG处理器增强，支持消息实时入库
- **配置热重载系统**：无需重启即可热更新配置
  - AsyncIOEventBus 事件总线（优先级订阅、超时保护）
  - FileWatcher 文件监控（500ms防抖）
  - ConfigManager 配置管理器（原子性重载、自动回滚）
  - ConfigValidator 配置验证器（详细错误报告）
- **自动趣味投票配置**：新增 `/set_auto_poll` 命令，为每个频道单独配置AI自动趣味投票
- **转发规则持久化**：新增转发规则配置持久化功能，支持通过命令动态管理
- **转发底栏增强**：支持助推和助手BOT链接，支持自定义底栏模板变量
- **QA时间上下文**：为QA提示词添加当前时间上下文，提升回答准确性
- **频道评论欢迎优化**：优化频道评论欢迎逻辑和prompt配置

### 改进
- **统一配置加载逻辑**：统一配置加载逻辑并优化自动投票处理器
- **配置热重载布尔值解析**：修复配置项热重载时的布尔值类型解析问题
- **投票公开/匿名配置**：支持投票公开/匿名配置并抽取公共发送逻辑
- **频道标识符获取**：提取频道标识符获取逻辑并清理prompt文件
- **模块目录结构重构**：重构模块目录结构，代码更加模块化和可维护
...
### 新增
- **投票公开/匿名配置**：为每个频道单独配置投票为公开或匿名模式
- **public_voters 配置增强**：增强 public_voters 配置对字符串类型的转换支持

### 改进
- **模块化重构**：main.py 模块化重构，代码量减少 80%
- **命令分类优化**：优化命令分类结构并添加国际化支持
- **日志系统重构**：统一管理配置并支持彩色输出
...
### 新增
- **数据库清空命令**：恢复数据库清空命令及底层实现
- **转发功能持久化**：实现转发功能的持久化配置和规则管理
- **转发规则管理命令**：新增转发规则管理命令及辅助函数

### 改进
- **事件总线重构**：重构事件总线并新增自定义异常体系
- **配置热重载**：新增提示词变更事件与文件监控支持
- **版本号升级**：更新版本号至 1.7.5
...
### 新增
- **配置热重载完整实现**：实现完整的配置热重载系统
  - AsyncIOEventBus 事件总线
  - FileWatcher 文件监控（500ms防抖）
  - ConfigManager 配置管理器（原子性重载）
  - ConfigValidator 配置验证器
  - Telegram错误通知器（支持重试按钮）
- **端到端集成测试**：添加端到端集成测试

### 改进
- **配置自动回滚**：实现配置自动回滚机制并优化错误通知
- **转发配置热重载**：实现转发配置热重载功能
- **系统配置管理器**：引入系统配置管理器并支持配置热重载
...
### 新增
- **查询改写功能增强**：引入多策略改写机制和智能缓存系统
  - **改写策略枚举类 RewriteStrategy**：定义四种改写策略
    - 代词解析：专门处理包含代词（它、这个、那个等）的查询
    - 查询扩展：基于上下文扩展查询内容，增加信息量
    - 查询简化：移除冗余词汇，提炼核心意图
    - 混合增强：综合运用多种策略，提供最佳改写效果
  
  - **查询改写缓存类 RewriteCache**：高性能缓存机制
    - LRU 淘汰策略：自动清理最少使用的缓存条目
    - 命中率统计：实时监控缓存效果，优化性能
    - 线程安全设计：支持并发访问，避免数据竞争
    - 智能缓存键：基于查询和历史生成唯一键值
  
  - **Few-Shot Learning 示例**：增强系统提示词
    - 提供真实改写示例，提高 AI 改写准确性
    - 覆盖多种查询场景（代词指代、关键词补充等）
    - 动态示例生成，根据对话历史定制化
  
  - **智能改写决策**：新增 _should_rewrite 方法
    - 基于对话历史判断是否需要改写
    - 检测代词使用情况
    - 分析关键词数量和复杂度
    - 自动选择最优改写策略
  
  - **改写方法优化**：_rewrite_query_enhanced 方法增强
    - 支持多策略改写，根据场景自动切换
    - 集成缓存查询，提升响应速度
    - 详细的日志记录，便于调试和监控
    - 优雅的降级机制，改写失败时使用原始查询
...
### 技术实现
- **核心模块更新**：`core/qa_engine_v3.py`
  - 新增 `RewriteStrategy` 枚举类
  - 新增 `RewriteCache` 类
  - 增强 `_should_rewrite()` 方法
  - 优化 `_rewrite_query_enhanced()` 方法
...
### 使用场景
- **代词解析**：用户问"它有什么特点？"时自动改写为"Claude 3有什么特点？"
- **查询扩展**：用户问"最近动态"时改写为"最近2天频道内的重要讨论"
- **查询简化**：用户问"那个我很感兴趣的上周五下午三点钟关于人工智能发展趋势的讨论结果怎么样了"时简化为"上周五AI发展趋势讨论结果"
- **混合增强**：综合运用多种策略，提供最佳改写效果
