...
- **CI提交信息处理错误**：修复了提交信息过长或包含换行符导致的 bash 脚本错误
  - **问题**：当提交信息过长（>200字符）或包含换行符时，CI 检查脚本报错 "File name too long"
  - **原因**：bash 脚本直接使用原始提交信息，没有清理多余的空格和换行符
  - **影响**：导致 CI 工作流失败，PR 无法合并
  - **修复**：在获取提交信息后使用 `tr`、`sed` 和 `grep` 清理输入
  - **结果**：现在可以正确处理各种格式的提交信息，避免脚本错误
